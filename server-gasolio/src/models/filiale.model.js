...
Filiale.updateById = (id, filiale, result) => {
  sql.query(
    "UPDATE filiale SET nomeFiliale = ?, indirizzo = ?, citta = ?, provincia = ?, cap = ? WHERE id = ?",
    [filiale.nomeFiliale, filiale.indirizzo, filiale.citta, filiale.provincia, filiale.cap, id],
    (err, res) => {
      if (err) {
        console.log("error: ", err);
        result(null, err);
